...
for d in ["train", "val"]:
    DatasetCatalog.register("retina-mots" + d, lambda d=d: get_MOTS_dicts(d))
    MetadataCatalog.get("retina-mots" + d).set(thing_classes=['Car', 'Pedestrian', 'DontCare'])
kitti_metadata = MetadataCatalog.get("retina-motstrain")

dataset_dicts = get_MOTS_dicts('train')

cfg = get_cfg()
cfg.merge_from_file("./detectron2_repo/configs/COCO-Detection/retinanet_R_101_FPN_3x.yaml")
cfg.DATASETS.TRAIN = ("retina-motstrain",)
...
cfg.DATASETS.TEST = ("retina-motsval", )


evaluator = COCOEvaluator("retina-motsval", cfg, False, output_dir="./output/")
